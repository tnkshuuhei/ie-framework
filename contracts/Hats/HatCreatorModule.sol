...
contract HatsHatCreatorModule is HatsModule, IHatsHatCreatorModule {
    ...
    function _authorized(address authority) internal view returns (bool) {
        return HATS().isAdminOfHat(authority, creatorHatId) || HATS().isWearerOfHat(authority, creatorHatId);
    }
        ...
        returns (uint256)
    {
        ...
        return HATS().createHat(_admin, _details, _maxSupply, _eligibility, _toggle, _mutable, _imageURI);
    }
     ...
     */
    function changeHatDetails(uint256 hatId, string calldata newDetails) external override {
        require(hasAuthority(msg.sender), "Not authorized");
     ...
     * @dev Only callable by addresses with hat creation authority
     */
    function changeHatImageURI(uint256 hatId, string calldata newImageURI) external override {
        require(hasAuthority(msg.sender), "Not authorized");
     ...
     * @dev Only callable by addresses with hat creation authority
     */
    function changeHatMaxSupply(uint256 hatId, uint32 newMaxSupply) external override {
        require(hasAuthority(msg.sender), "Not authorized");
